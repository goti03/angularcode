...
import { gemConstant,nonSoleProp,sellerConstant,retailerConstant } from '../../../core/constant';
import { breadcrumbMessage} from '../../../shared/breadcrumb-message.service'
import {Crypto} from '../../../shared/crypto.service';

@Component({
  selector: 'app-financial-processing',
  templateUrl: './financial-processing.component.html',
  styleUrls: ['./financial-processing.component.css']
})
export class FinancialProcessingComponent implements OnInit {
  ...
  changeFileType(){
  var fileTypes=((document.getElementById("fileType")) as HTMLInputElement).value;
    if(fileTypes == 'pdf'){
      this.addRowButton = false;
      
    }else if(fileTypes == 'xls'){
      this.addRowButton = true;
  ...
  process() {
    ...
    for (let i = 0; i < checkbox.length; i++) {
      ...
      if (value == true) {
        ...
        // var bankNameTemp = this.UploadedDocuments[i].bankName.split(",");
        // var accTypeTemp = this.UploadedDocuments[i].acctTypeName.split(",");
        // var processStatusTemp = this.UploadedDocuments[i].processStatus.split(",");

        for (let j = 0; j < fileNameTemp.length; j++) {
          // if (processStatusTemp[i] == 0) {
            const data1 = {
  ...
  submit() {
    var filetype=((document.getElementById("fileType")) as HTMLInputElement).value;
  ...
  save(){
    this.submitted = true;
    // alert("Please wait, we are processing the bank statement");
    this.curDate = moment().format('YYYY-MM-DD HH:mm:ss');
    var bankname = [];
    var accName = [];
    var fileContent = [];
    ...
    for (var i = 0; i < name1.length; i++) {
      ...
      var fileName = this.loanid+"_BankStatement_"+moment().format('YYYYMMDDHHmmss_')+i+"."+fileType;
      // alert("fileName::"+fileName);
      // alert("fileNameList:"+JSON.stringify(this.fileNameList));
      // alert("file:"+JSON.stringify(this.imageEncode[i].split(",")[1]));

      // this.fileDynamicList[i].fileName=this.fileNameList[i].fileName;
    ...
    if (fileType == 'pdf' && this.duplicate==0) {
      // this.apiService.getParsedBankTransactions(bankstatementData).subscribe(data => {
      this.apiService.uploadBankstatements(bankstatementData).subscribe(data => {
      ...
      this.apiService.getParsedBankStatementExcel(bankstatementData).subscribe(data => {
        ...
        if (data.status == 200) {
          ...
          console.log("this.list1::"+JSON.stringify(this.List1));
          console.log("this.fiarray::"+JSON.stringify(this.fiarray));
